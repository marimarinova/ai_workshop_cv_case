...
def events_from_rows(rows, column_map=None):
    ...
    return [EvaluationEvent(
        clip_id=str(_get(r, "clip_id", cm)), type=str(_get(r, "type", cm)),
        t_start=float(_get(r, "t_start", cm)), t_end=float(_get(r, "t_end", cm)),
...
def ignores_from_rows(rows, column_map=None):
    cm = column_map or {}
    return [EvaluationIgnoreInterval(clip_id=str(_get(r, "clip_id", cm)),
                                     t_start=float(_get(r, "t_start", cm)),
                                     t_end=float(_get(r, "t_end", cm))) for r in rows]
