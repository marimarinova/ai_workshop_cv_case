...
def render_markdown(metrics, model_name="model"):
    ...
    for k, v in metrics.items():
        if k == "confusion":
            lines += ["## Type confusion", "| gt \\ pred | " + " | ".join(VALID_TYPES) + " |",
                      "|" + "---|" * (len(VALID_TYPES) + 1)]
            for a in VALID_TYPES:
                lines.append(f"| {a} | " + " | ".join(str(v[a][b]) for b in VALID_TYPES) + " |")
        elif isinstance(v, dict):
            lines.append(f"## {k}")
            lines.append(", ".join(f"{kk}={_fmt(vv)}" for kk, vv in v.items()
                                   if not isinstance(vv, dict)))
            for kk, vv in v.items():
                if isinstance(vv, dict):
                    lines.append(f"- {kk}: " + ", ".join(f"{k2}={_fmt(v2)}" for k2, v2 in vv.items()))
        else:
            lines.append(f"- **{k}**: {v}")
        lines.append("")
    return "\n".join(lines)
...
def render_html(metrics, model_name="model"):
    ...
    for k, v in metrics.items():
        if k == "confusion":
            ...
            parts.append(f"<h2>Type confusion</h2><table border='1' cellpadding='4'><tr><th>gt \\ pred</th>{head}</tr>")
            for a in VALID_TYPES:
    ...
    return "<!doctype html><html><head><meta charset='utf-8'>" \
           f"<title>Evaluation report - {esc(str(model_name))}</title></head><body>" + "".join(parts) + "</body></html>"
...
def failure_gallery(events, preds, criterion=None, ignores=()):
    ...
    ev = drop_ignored(events, ignores); pr = drop_ignored(preds, ignores)
    ...
    for clip in set(ge) | set(gp):
        ...
        for g, p in m.matched:
            if type_name(g.type) != type_name(p.type):
                type_confusions.append({"clip_id": g.clip_id, "gt_type": type_name(g.type),
                                        "pred_type": type_name(p.type), "t_start": g.t_start, "t_end": g.t_end})
    return {
        "false_positives": [{"clip_id": p.clip_id, "type": type_name(p.type), "t_start": p.t_start, "t_end": p.t_end, "score": getattr(p, "score", None)} for p in r.unmatched_pred],
        "false_negatives": [{"clip_id": g.clip_id, "type": type_name(g.type), "t_start": g.t_start, "t_end": g.t_end, "hard_case": getattr(g, "hard_case", None)} for g in r.unmatched_gt],
