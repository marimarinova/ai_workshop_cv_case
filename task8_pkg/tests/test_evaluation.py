...
from pickup_putdown.evaluation import (
    ...
    aggregate_metrics, average_precision, evaluate_class_aware, evaluate_confusion,
    events_from_rows, failure_gallery, match_one_to_one, match_ranked, mean_ap,
    metrics_to_json, midpoint_distance, predictions_from_rows, render_html,
    render_markdown, slice_metrics, tiou,
)
...
def test_lexicographic_matches_brute_force():
    ...
    for _ in range(3000):
        def mk(i, cls):
            s = round(rng.uniform(0, 3), 2); e = s + round(rng.uniform(0.5, 2), 2)
...
def test_ignore_any_overlap_excluded():
    ...
    r = evaluate_class_aware([Event("c", "pickup", 1.0, 2.0)], [Prediction("c", "pickup", 1.0, 2.0)],
                             Criterion("tiou", 0.5), ignores=[IgnoreInterval("c", 1.5, 5.0)])
    assert (r.tp, r.fp, r.fn) == (0, 0, 0)

def test_midpoint_criterion_matches_when_tiou_low():
    gts = [Event("c", "pickup", 0.0, 1.0)]; pr = [Prediction("c", "pickup", 0.6, 1.6)]
...
def test_greedy_matcher_order_invariant_and_matches_hungarian():
    gts = [Event("c", "pickup", 0.0, 1.0, "g1"), Event("c", "pickup", 10.0, 11.0, "g2")]
    pr = [Prediction("c", "pickup", 0.0, 1.0, "p1", score=0.7), Prediction("c", "pickup", 10.0, 11.0, "p2", score=0.9)]
...
def test_multi_item_default_group_id():
    gts = [Event("c", "pickup", 1.0, 2.0, "g1", group_id="A"),
           Event("c", "pickup", 5.0, 6.0, "g2", group_id="A")]  # different times, shared group
    pr = [Prediction("c", "pickup", 1.0, 2.0, "p1")]
...
def test_multi_item_overlap_is_opt_in():
    gts = [Event("c", "pickup", 0.0, 2.0, "g1"), Event("c", "pickup", 1.0, 3.0, "g2")]  # tIoU=1/3, no group
    pr = [Prediction("c", "pickup", 0.0, 2.0, "p1")]
    assert aggregate_metrics(gts, pr, {"c": 100.0})["multi_item_recall"] is None         # default: not multi
    assert aggregate_metrics(gts, pr, {"c": 100.0}, multi_item_overlap_thr=0.3)["multi_item_recall"] == 0.5


# --- aggregate / AP ---
def test_aggregate_metrics_bundle():
    gts = [Event("c", "pickup", 1.0, 2.0), Event("c", "putdown", 5.0, 6.0)]
    pr = [Prediction("c", "pickup", 1.0, 2.0, score=0.9), Prediction("c", "putdown", 5.1, 6.1, score=0.8)]
...
def test_ap_perfect_and_ranking_and_undefined():
    ...
    good = [Prediction("c", "pickup", 1.0, 2.0, score=0.9), Prediction("c", "pickup", 50.0, 51.0, score=0.5)]
    bad = [Prediction("c", "pickup", 50.0, 51.0, score=0.9), Prediction("c", "pickup", 1.0, 2.0, score=0.5)]
...
def test_reports_and_json_serializable():
    gts = [Event("c", "pickup", 1.0, 2.0), Event("c", "putdown", 5.0, 6.0)]
    pr = [Prediction("c", "pickup", 1.0, 2.0, score=0.9), Prediction("c", "putdown", 5.0, 6.0, score=0.8)]
    m = aggregate_metrics(gts, pr, {"c": 600.0})
...
def test_slice_metrics_recall_oriented():
    gts = [Event("c", "pickup", 1.0, 2.0, confidence="low"), Event("c", "putdown", 5.0, 6.0, confidence="high")]
    pr = [Prediction("c", "pickup", 1.0, 2.0)]
    sl = slice_metrics(gts, pr, {"c": 600.0})
    assert "all" in sl
    assert set(sl["low_confidence"]) == {"recall", "tp", "fn", "support", "threshold"}  # recall-only + the threshold used
...
def test_io_preserves_zero_score_and_column_map():
    ...
    p = predictions_from_rows([{"clip_id": "c", "type": "pickup", "t_start": "1", "t_end": "2", "score": "0"}])
    assert p[0].score == 0.0   # 0.0 preserved, not coerced to 1.0

def test_runtime_per_video_minute():
    gts = [Event("c", "pickup", 1.0, 2.0)]; pr = [Prediction("c", "pickup", 1.0, 2.0)]
    assert aggregate_metrics(gts, pr, {"c": 600.0}, runtime_s=30.0)["runtime_per_video_minute"] == 3.0
    assert aggregate_metrics(gts, pr, {"c": 600.0})["runtime_per_video_minute"] is None

def test_failure_gallery_type_confusions():
    gts = [Event("c", "pickup", 1.0, 2.0)]; pr = [Prediction("c", "putdown", 1.0, 2.0)]
    g = failure_gallery(gts, pr, Criterion("tiou", 0.5))
    assert g["type_confusions"][0]["gt_type"] == "pickup" and g["type_confusions"][0]["pred_type"] == "putdown"
...
def test_map_order_invariant():
    ...
    p = [Prediction("c", "pickup", 1.0, 2.0, "p1", score=0.6), Prediction("c", "pickup", 2.2, 4.2, "p2", score=0.9)]
    assert average_precision(g, p, "pickup", 0.5) == average_precision(list(reversed(g)), list(reversed(p)), "pickup", 0.5)


def test_minimal_objects_do_not_crash():
    class Min:
        def __init__(s, c, t, a, b):
            s.clip_id, s.type, s.t_start, s.t_end = c, t, a, b
    gm = [Min("c", "pickup", 1.0, 2.0)]; pm = [Min("c", "pickup", 1.0, 2.0)]
...
def test_enum_backed_types_accepted():
    ...
    class O:
        def __init__(s):
            s.clip_id, s.type, s.t_start, s.t_end, s.score = "c", T.PICKUP, 1.0, 2.0, 0.9
    assert aggregate_metrics([O()], [O()], {"c": 10.0})["tiou@0.5"]["tp"] == 1


def test_html_report_escapes_model_name():
    m = aggregate_metrics([Event("c", "pickup", 1.0, 2.0)], [Prediction("c", "pickup", 1.0, 2.0)], {"c": 10.0})
