"""Hand carrying-state classifier for Track A (task_10).

Inference-time interface behind :class:`HandStateClassifier`. A trained
estimator (fit in task_7) is loaded from ``TrackAConfig.hand_state_checkpoint``
when present. Until then :func:`load_hand_state_classifier` returns a
...
from pickup_putdown.layer1.track_a.state_types import Embedding, HandStateClassifier

logger = logging.getLogger(__name__)


def is_available(config: TrackAConfig) -> bool:
    """Return ``True`` only when a trained hand-state checkpoint exists on disk."""
    return Path(config.hand_state_checkpoint).is_file()
...
class PlaceholderHandStateClassifier:
    ...
    def predict_holding(self, embedding: Embedding) -> float:
        """Heuristic ``P(holding)`` from the embedding mean (placeholder only)."""
        return _sigmoid(float(np.mean(embedding)))


class TrainedHandStateClassifier:
...
def load_hand_state_classifier(config: TrackAConfig) -> HandStateClassifier:
    """Return the trained classifier if a checkpoint exists, else the placeholder."""
    if is_available(config):
        return TrainedHandStateClassifier(config.hand_state_checkpoint)
    logger.warning(
        "No hand-state checkpoint at %s; using PLACEHOLDER classifier — outputs "
        "are NOT validated (task_7 pending).",
        config.hand_state_checkpoint,
    )
    return PlaceholderHandStateClassifier()
