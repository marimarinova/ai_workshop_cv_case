...
### Task 6.1 metadata format

Task 6.1 produces **one metadata JSON file per source video**. Each file
contains source-level metadata and zero or more candidate records nested
under `.candidates[]`. Files are stored recursively:

```text
.local/candidate_staging/candidates/
└── <source_video_id>/
    └── <source_video_id>.json
```

Example source-level metadata file:

```json
{
  "source_video_id": "D2_S20260520135131_E20260520135549_anon",
  "source_bucket": "chillnbite-cameras",
  "source_key": "anon/source/example.mp4",
  "candidate_count": 2,
  "candidates": [
    {
      "candidate_id": "cand_3f54241aeb53",
      "candidate_key": ".local/candidate_staging/candidates/.../cand_3f54241aeb53.mp4",
      "source_start_s": 0.0,
      "source_end_s": 3.75,
      "duration_s": 3.75,
      "codec": "h264",
...
Task 6.2 flattens these nested records internally. You do **not** need to
manually flatten the metadata before running the task-generation command.

Source files with zero candidates (`"candidate_count": 0` or an empty
`"candidates"` array) are skipped silently. They do not produce tasks,
errors, or warnings.

**Backward compatibility:** The loader also accepts flat candidate metadata
(one JSON object per file, or a JSON array of candidates). Format is
detected explicitly from the JSON structure: presence of a `"candidates"`
key signals source-level format; its absence signals flat format.

### Required candidate fields

Each nested candidate record must include:

| Field | Required | Description |
|---|---|---|
| `candidate_id` | yes | Unique candidate identifier |
| `candidate_key` | yes | S3 key, URL, or local path to candidate video |
...
The following fields are inherited from the source-level metadata
automatically (no need to repeat per candidate):

| Inherited Field | Source | Description |
|---|---|---|
| `clip_id` | `source_video_id` | Original source video identifier |
| `source_bucket` | parent | S3 bucket containing source video |
| `source_key` | parent | S3 key of the source video |

Optional candidate fields (preserved but not required):

| Field | Description |
|---|---|
| `actor_id` | Actor track identifier (e.g. `track_3`) |
| `hand_side` | Hand side (`left` or `right`) |
| `region_id` | Shelf/surface region identifier |
| `proposal_score` | Candidate proposal score |
| `config_fingerprint` | Generation configuration fingerprint |
| `duration_s` | Candidate clip duration in seconds |
| `fps` | Candidate clip frame rate |
| `codec` | Video codec |
| `pixel_format` | Pixel format |

Missing optional fields do not prevent task creation. Missing required fields
produce a clear validation error naming the file, source video ID, candidate
...
The `candidate_key` field in the metadata must point to a location Label
Studio can access. When using `--video-url-mode s3_storage`, local video
files are **not required** — Label Studio reads directly from S3 via its
...
pickup-putdown annotation-build-tasks \
  --candidate-metadata-dir .local/candidate_staging/candidates \
...
With S3 storage integration (no local video files needed):
...
The command:
- Recursively discovers all `.json` files under the metadata directory
  (sorted deterministically by path).
- Flattens source-level metadata into individual candidate records internally.
- Inherits `source_video_id` as `clip_id`, plus `source_bucket` and
  `source_key` from the parent source file.
- Skips source files with zero candidates silently.
- Produces one Label Studio task per valid candidate.
- With `--limit`, selects a deterministic subset using `--seed` for
  reproducibility.
- Reports source files scanned, zero-candidate sources skipped, candidates
  loaded, and tasks generated.
- Fails with exit code 1 when any candidate has validation errors or
  metadata load errors (malformed JSON, missing required fields, duplicate
  candidate IDs).
